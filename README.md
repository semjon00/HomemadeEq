...
```java -jar uber-apk-signer-1.3.0.jar -a C:\Users\semjon\Desktop\123\Wavelet+v24.05+(2168).apk```. Or just, you know,
...
*Update 2025-02-03* The Personal Pink "Pinkie Pie" headphones bought for one euro got a slightly better treatment.
There is a `measure.py` for getting a diff against the reference. Idk it looked a bit broken, but there was an attempt.
`clomper.py` for converting some arbitrary curves (Equalizer APO?) to wavelet curves.